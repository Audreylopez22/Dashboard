...
st.title("🎓 Dashboard de egresados")
...
if 'master_df' in st.session_state:
    ...
    if job_col in original_df.columns:
        job_options = ["Todos"] + sorted(original_df[job_col].dropna().unique().tolist())
        job_status = st.sidebar.selectbox("¿Trabaja actualmente?", job_options)
        if job_status != "Todos":
    ...
    if studies_col in original_df.columns:
        studies_options = ["Todos"] + sorted(original_df[studies_col].dropna().unique().tolist())
        studies_status = st.sidebar.selectbox("¿Continuó con sus estudios?", studies_options)
    ...
    if startup_col in original_df.columns:
        startup_options = ["Todos"] + sorted(original_df[startup_col].dropna().unique().tolist())
        startup_status = st.sidebar.selectbox("¿Ha creado empresa o emprendimiento?", startup_options)
        ...
        if filtered_df.empty:
            ...
            with col_ac2:
                ...
                fig_niv.update_layout(
                    showlegend=False,
                    xaxis={'categoryorder':'total descending'}, # Pone el más alto a la izquierda
                    xaxis_title="",
    ...
    with tab4: 
        if filtered_df.empty:
            ...
            if unit_col in filtered_df.columns and not filtered_df.empty:
                exclude_list = ['N/A', 'Otro', 'Otros', 'No reporta', 'Sin información']
                df_plot = filtered_df[~filtered_df[unit_col].astype(str).isin(exclude_list)].copy()
                unit_data = df_plot[unit_col].value_counts().reset_index()
                unit_data.columns = [unit_col, 'Cantidad']
                n_valid = len(df_plot)
                ...
                col_name = '¿Cuál es la actividad económica del emprendimiento o empresa?'
                if col_name in filtered_df.columns:
                    exclude_list = ['N/A', 'Otro', 'Otros', 'No reporta', 'Sin información']
                    df_plot = filtered_df[~filtered_df[col_name].astype(str).isin(exclude_list)].copy()
                    data_act = df_plot[col_name].value_counts().reset_index()
                    n_valid = len(df_plot)
            ...
            if column_canales in filtered_df.columns:
                    df_plot = filtered_df[~filtered_df[column_canales].astype(str).isin(exclude_list)].copy()
                    # En canales múltiples, el n_valid es sobre personas que marcaron al menos una opción
                    n_valid = len(df_plot)
                    
                    series_canales = df_plot[column_canales].dropna().astype(str).str.split(',')
                    all_canales = [item.strip() for sublist in series_canales for item in sublist if item.strip()]
                    
                    if all_canales:
                        st.caption(f"Muestra: {n_valid} de {n_total} empresas reportadas")
